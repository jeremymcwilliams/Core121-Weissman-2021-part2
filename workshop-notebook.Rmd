...
Now create a variable "myData", and assign it to the output of the "formatFile()" function, passing your file name as the argument"
...
```{r}
manualData
ggplot(data=myData, mapping=aes(x="",y=timesEmailChecked))+
  geom_boxplot()+
...
boxData <- data.frame(
  ...
  Variable=c("Texts to Mom","Times Checked Email","Texts to Mom","Times Checked Email","Texts to Mom","Times Checked Email"),
  Value=c(4,2,8,3,10,4)
)

boxData

ggplot(data=boxData, mapping=aes(x=Variable, y=Value, fill=Variable)) +
  geom_boxplot()+
  stat_summary(fun=mean, geom="point", shape=20, size=5, color="red", fill="red")
